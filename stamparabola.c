...
int main() {
	...
	BuildMap(tempmap);
	int tankx = 48;
	int tanky = 220, dx, x, real_y, temp_y, direction = 4;
...
/*
		...
		if (tempmap[x][real_y] == '|' || x < 0 || x >= N || real_y < 0 || real_y >= M)
		{
			printf("your shot is put of range");
			y = 1000;
			break;
		}
		if (y != 1000)
...
int main() {
	...
	BuildMap(tempmap);
	int tankx = 48;
	int tanky = 150, dx, x, real_y, temp_y, direction;
	do
	{
		printf("which direction do you want to shot? 1-Left  2-Right\n");
		scanf("%c", &d);
